...
export default function Whiteboard(props) {
  ...
  const [isToolLocked, setIsToolLocked] = React.useState(
    tldrawAPI?.appState?.isToolLocked
  )
  ...
  const toggleOffCheck = (evt) => {
    ...
    const panBtnClicked =
      clickedElement?.getAttribute('data-test') === 'panButton' ||
  ...
  React.useEffect(() => {
    if (
      bgShape &&
      bgShape.parentElement &&
      bgShape.parentElement.clientWidth > 0
    ) {
      bgShape.parentElement.style.width = `${
        bgShape.parentElement.clientWidth + 0.1
      }px`
    }
  }, [bgShape])
  ...
  const doc = React.useMemo(() => {
    ...
    if (
      ...
      currentDoc?.pages.length !== curPres?.pages.length
    ) {
    ...
    if (next.pageStates[curPageId] && !isEqual(prevShapes, shapes)) {
      const editingShape = tldrawAPI?.getShape(
        tldrawAPI?.getPageState()?.editingId
      )
      ...
      const removed =
        prevShapes && findRemoved(Object.keys(prevShapes), Object.keys(shapes))
      if (removed && removed.length > 0) {
        const patchedShapes = Object.fromEntries(
          removed.map((id) => [id, undefined])
        )
      ...
      next.pages[curPageId].shapes = filterInvalidShapes(
        shapes,
        curPageId,
        tldrawAPI
      )
    ...
    if (
      ...
      (!next.assets[`slide-background-asset-${curPageId}`] ||
        (svgUri && !isEqual(prevSvgUri, svgUri)))
    ) {
      next.assets[`slide-background-asset-${curPageId}`] =
    ...
    if (changed && tldrawAPI) {
      ...
      const patch = {
        document: {
          pages: {
            [curPageId]: {
              shapes: filterInvalidShapes(shapes, curPageId, tldrawAPI),
            },
          },
        },
      }
      ...
      if (nextState.document.pages[curPageId].shapes) {
        filterInvalidShapes(
          nextState.document.pages[curPageId].shapes,
          curPageId,
          tldrawAPI
        )
      }
    ...
    if (next.pages[curPageId] && slidePosition) {
      const pollResults = Object.entries(next.pages[curPageId].shapes).filter(
        ([, shape]) => shape.name?.includes('poll-result')
      )
        ...
        if (isEqual(shape.point, [0, 0])) {
          try {
            ...
            if (shapeBounds) {
              ...
              if (isPresenter)
                persistShape(editedShape, whiteboardId, isModerator)
            }
          } catch (error) {
            logger.error(
              {
    ...
    if (tldrawAPI && isPresenter && curPageId && slidePosition && !isMounting) {
      ...
      if (previousAspectRatio !== currentAspectRatio) {
        if (fitToWidth) {
          ...
          const zoomToolbar =
            Math.round(((HUNDRED_PERCENT * camera.zoom) / zoomFitSlide) * 100) /
            100
          if (zoom !== zoomToolbar) {
            setZoom(zoomToolbar)
  ...
  const fullscreenToggleHandler = () => {
    const { isFullscreen, fullscreenAction, handleToggleFullScreen } = props
  ...
  const onPatch = (e, t, reason) => {
    ...
    if (
      (isPanning || panSelected) &&
      (reason === 'selected' || reason === 'set_hovered_id')
    ) {
      e.patchState({
        document: {
          pageStates: {
    ...
    if (reason && reason.includes('selected')) {
      const validIds = []
      e.pageState.selectedIds.forEach(
        (id) => hasShapeAccess(id) && validIds.push(id)
      )
    ...
    if (
      e?.session?.type === 'translate' &&
      e?.session?.status === 'translating'
    ) {
    ...
    if (
      ...
      isPresenter &&
      slidePosition &&
      (reason.includes('zoomed') || reason.includes('panned'))
    ) {
      const camera = tldrawAPI?.getPageState()?.camera
      ...
      const zoomToolbar =
        Math.round(((HUNDRED_PERCENT * camera.zoom) / zoomFitSlide) * 100) / 100
      if (zoom !== zoomToolbar) {
        setZoom(zoomToolbar)
    ...
    if (reason && reason === 'patched_shapes' && e?.session?.type === 'edit') {
      ...
      if (
        e?.session?.initialShape?.type === 'sticky' &&
        patchedShape?.text?.length > maxStickyNoteLength
      ) {
        patchedShape.text = patchedShape.text.substring(0, maxStickyNoteLength)
      }

      if (
        e?.session?.initialShape?.type === 'text' &&
        !shapes[patchedShape.id]
      ) {
        // check for maxShapes
        const currentShapes = e?.document?.pages[e?.currentPageId]?.shapes
        const shapeNumberExceeded =
          Object.keys(currentShapes).length - 1 > maxNumberOfAnnotations
  ...
  const onUndo = (app) => {
    ...
    const changedShapes =
      lastCommand?.before?.document?.pages[app.currentPageId]?.shapes
    if (changedShapes) {
      sendShapeChanges(
        app,
        changedShapes,
  ...
  const onRedo = (app) => {
    ...
    const changedShapes =
      lastCommand?.after?.document?.pages[app.currentPageId]?.shapes
    if (changedShapes) {
      sendShapeChanges(
        app,
        changedShapes,
  ...
  const onCommand = (app, command) => {
    const isFirstCommand =
      command.id === 'change_page' &&
      command.before?.appState.currentPageId === '0'
    if (!isFirstCommand) {
    ...
    if (command?.id?.includes('style')) {
      setCurrentStyle({
        ...currentStyle,
        ...command?.after?.appState?.currentStyle,
      })
    }

    const changedShapes =
      command.after?.document?.pages[app.currentPageId]?.shapes
    if (!isMounting && app.currentPageId !== curPageId) {
      ...
      if (changedShapes) {
        ...
        Object.entries(changedShapes).forEach(([id, shape]) => {
  ...
  const backgroundShape = document.getElementById(
    'slide-background-shape_image'
  )

  if (currentTool && !isPanning && !tldrawAPI?.isForcePanning)
    tldrawAPI?.selectTool(currentTool)

  if (
    backgroundShape &&
    backgroundShape.src && // if there is a background image
  ...
  const size =
    height < SMALL_HEIGHT || width < SMALL_WIDTH ? TOOLBAR_SMALL : TOOLBAR_LARGE
